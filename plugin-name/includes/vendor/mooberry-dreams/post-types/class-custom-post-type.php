...
if ( !class_exists( 'MOOBD_Custom_Post_Type' ) ) {
	class MOOBD_Custom_Post_Type implements MOOBD_ICustom_Post_Type {
		...
		public function __construct( $post_type, $singular_name, $plural_name ) {
			...
			$this->args['labels'] = array(
				...
				'new_item'           => __( 'New', 'plugin-name' ) . ' ' . $this->singular_name,
				'edit_item'          => __( 'Edit', 'plugin-name' ) . ' ' . $this->singular_name,
				'view_item'          => __( 'View', 'plugin-name' ) . ' ' . $this->singular_name,
				'all_items'          => __( 'All ', 'plugin-name' ) . $this->plural_name,
				'search_items'       => __( 'Search', 'plugin-name' ) . ' ' . $this->plural_name,
				'parent_item_colon'  => sprintf( __( 'Parent %s:', 'plugin-name' ), $this->plural_name ),
				'not_found'          => sprintf( __( 'No %s found.', 'plugin-name' ), strtolower( $this->plural_name ) ),
				'not_found_in_trash' => sprintf( __( 'No %s found in Trash.', 'plugin-name' ), strtolower( $this->plural_name ) ),
			);
